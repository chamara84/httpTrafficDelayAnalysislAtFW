...
import pp
import WrokLoadForAllWithoutMainMerlinGenArrSMTP
#import matplotlib.pyplot as plt  
#from decimal import *


def simulate(scalePar, meanInterArr,lines,scores):
	...
	packetID = 0
	numberOfcores = 1
	globalVar =WrokLoadForAllWithoutMainMerlinGenArrSMTP.Global_var()
	...
	arrProcess =WrokLoadForAllWithoutMainMerlinGenArrSMTP.Arrival(arrtime,contentType,size,packetID,scaleParSim,globalVar,lines,scores)
...
def main():
	...
	[scalePar,utilization,meanInterArr,lines] = getScalePar()
	...
	for scalePar in scaleParVec:
		[scale,utilization,meanInterArr,lines] = getScalePar()
		data = list([])
		workLoad =list([])
		queueLength = list([])
		queueSizeInNum = list([])
		...
		if scalePar <= 1.0:
			for lineNum in range(len(lines)):
				if rand.uniform()<=float(scalePar):
			...
			startTime = lines[0][0]
			print 'End Time Lines', str(lines[-1][0])
			for lineNum in range(len(lines)):
				linesScaled[lineNum][0] = float(linesScaled[lineNum][0])%((float(linesScaled[-1][0])-float(startTime))/float(scalePar))	
		...
		linesScaled = numpy.array(linesScaled,dtype={'names':('ArrTime','type','size'),'formats':('f8','S10','i8')})			
		interArrivalTimeVec = [(float(linesScaled[index+1][0])-float(linesScaled[index][0])) for index in range(len(linesScaled)-1)]
		interArrivalTimeVec.insert(0,float(linesScaled[0][0]))
		percentileVec = [range(1001)[index]*0.1 for index in range(1001)]
		
		for percentile in  percentileVec:
			scores.append(stats.scoreatpercentile(interArrivalTimeVec,percentile))
		...
		for numleft in range(int(numberOfparallelSims)):
			simInstances.append(jobServer.submit(simulate,(scalePar,meanInterArr,linesScaled,scores),(),("SimPy.Simulation","numpy","numpy.random","scipy.stats","scipy.constants","WrokLoadForAllWithoutMainMerlinGenArrSMTP","csv","string" )))
	...
	file = open('/Volumes/Home/Python_scripts/NetBeansProjects/antispamCategorize/src/workloadInfomationMerlinGenArrSMTP.txt','wb')
	w = csv.writer(file)
	...
	file = open('/Volumes/Home/Python_scripts/NetBeansProjects/antispamCategorize/src/queueLengthInfomationMerlinGenArrSMTP.txt','wb')
	w = csv.writer(file)
	w.writerow(utilizationVec)
	...
	file = open('/Volumes/Home/Python_scripts/NetBeansProjects/antispamCategorize/src/waitInfomationSMerlinGenArrSMTP.txt','wb')
	w = csv.writer(file)
	w.writerow(utilizationVec)
	...
	file = open('/Volumes/Home/Python_scripts/NetBeansProjects/antispamCategorize/src/queueSizeInNumberMerlinGenArrSMTP.txt','wb')
	w = csv.writer(file)
	w.writerow(utilizationVec)
	...
	file = open('/Volumes/Home/Python_scripts/NetBeansProjects/antispamCategorize/src/service_ArrTimeMerlinGenArrSMTP.txt','wb')
	w = csv.writer(file)
	w.writerow(utilizationVec)
	...
	file = open('/Volumes/Home/Python_scripts/NetBeansProjects/antispamCategorize/src/probWaitGret10msMerlinGenArrSMTP.txt','wb')
	w = csv.writer(file)
	w.writerow(utilizationVec)
...
def getScalePar():
	#file = open('/Users/umdevana/Documents/Python_scripts/ArrivalInfomationAnonNew.csv','rb')
	lines=numpy.genfromtxt('/Volumes/Home/Python_scripts/NetBeansProjects/antispamCategorize/src/ArrivalInfomationSMTPTRMerlin.csv',dtype={'names':('ArrTime','type','size'),'formats':('f8','S10','i8')},delimiter=',',missing_values=[0.0,'unknown','None',0],skip_header = 0)
	...
	serviceTimeFunc =WrokLoadForAllWithoutMainMerlinGenArrSMTP.ServiceProcess() 
	globalVar =WrokLoadForAllWithoutMainMerlinGenArrSMTP.Global_var()
	for index in range(len(lines)):
		...
		if index == 0:
			...
			serviceTime = serviceTime + serviceTimeFunc.serviceTime(lines[index][1],lines[index][2],globalVar)
			serviceTimeSquare = serviceTimeSquare + (serviceTimeFunc.serviceTime(lines[index][1],lines[index][2],globalVar))**2
	...
	utilization = serviceTime/(float(lines[-1][0])-float(lines[0][0]))
	...
	return [scale,utilization,meanInterArr,lines]

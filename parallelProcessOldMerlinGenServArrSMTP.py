...
import pp
import WrokLoadForAllWithoutMainMerlinGenServArrSMTP
#import matplotlib.pyplot as plt  
#from decimal import *


def simulate(scalePar, meanInterArr,ArrScores,scoresService,numLines,lines):
	scaleParSim = scalePar
	packetID = 0
	numberOfcores = 1
	globalVar =WrokLoadForAllWithoutMainMerlinGenServArrSMTP.Global_var()
	...
	arrtime=0.0
	arrProcess =WrokLoadForAllWithoutMainMerlinGenServArrSMTP.Arrival(arrtime,packetID,scaleParSim,globalVar,ArrScores,scoresService,lines)
	globalVar.process.activate(arrProcess,arrProcess.run(globalVar,numLines),at=globalVar.process.now() )
...
def main():
	...
	for scalePar in scaleParVec:
		...
		if scalePar<=1.0:
			...
			for lineNum in range(len(lines)):
				linesScaled[lineNum][0] = float(lines[lineNum][0])%(((float(lines[-1][0])-float(lines[0][0]))/float(scalePar)))
		...
		numLines = len(linesScaled)			
		interArrivalTimeVec = [max(float(linesScaled[index+1][0])-float(linesScaled[index][0]),0.0) for index in range(len(linesScaled)-1)]
		...
		for percentile in  percentileVec:
			ArrScores.append(stats.scoreatpercentile(interArrivalTimeVec,percentile))
			
		ArrScores = numpy.array(ArrScores,dtype=float)
		interArrivalTimeVec = []
		##end new code
		print str(scalePar)
		
		for numleft in range(int(numberOfparallelSims)):
			simInstances.append(jobServer.submit(simulate,(scalePar,meanInterArr,ArrScores,scoresService,numLines,linesScaled),(),("SimPy.Simulation","numpy","numpy.random","scipy.stats","scipy.constants","WrokLoadForAllWithoutMainMerlinGenServArrSMTP","csv","string" )))
	...
	file = open('/Volumes/Home/Python_scripts/NetBeansProjects/antispamCategorize/src/workloadInfomationMerlinGenServArrSMTP.txt','wb')
	w = csv.writer(file)
	...
	file = open('/Volumes/Home/Python_scripts/NetBeansProjects/antispamCategorize/src/queueLengthInfomationMerlinGenServArrSMTP.txt','wb')
	w = csv.writer(file)
	w.writerow(utilizationVec)
	...
	file = open('/Volumes/Home/Python_scripts/NetBeansProjects/antispamCategorize/src/waitInfomationMerlinGenServArrSMTP.txt','wb')
	w = csv.writer(file)
	w.writerow(utilizationVec)
	...
	file = open('/Volumes/Home/Python_scripts/NetBeansProjects/antispamCategorize/src/queueSizeInNumberMerlinGenServArrSMTP.txt','wb')
	w = csv.writer(file)
	w.writerow(utilizationVec)
	...
	file = open('/Volumes/Home/Python_scripts/NetBeansProjects/antispamCategorize/src/service_ArrTimeMerlinGenServArrSMTP.txt','wb')
	w = csv.writer(file)
	w.writerow(utilizationVec)
	...
	file = open('/Volumes/Home/Python_scripts/NetBeansProjects/antispamCategorize/src/probWaitGret10msMerlinServArrSMTP.txt','wb')
	w = csv.writer(file)
	w.writerow(utilizationVec)
...
def getScalePar():
	...
	serviceTimeFunc =WrokLoadForAllWithoutMainMerlinGenServArrSMTP.ServiceProcess() 
	serviceTimeVec = list([])
	scoresService = list([])
	globalVar =WrokLoadForAllWithoutMainMerlinGenServArrSMTP.Global_var()

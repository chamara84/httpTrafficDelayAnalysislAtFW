...
import pp
import WrokLoadForAllWithoutMain
...
def simulate(scalePar, meanInterArr,lines):
	...
	packetID = 0
	numberOfcores = 1
	globalVar = WrokLoadForAllWithoutMain.Global_var()
	...
	arrProcess = WrokLoadForAllWithoutMain.Arrival(arrtime,contentType,size,packetID,scaleParSim,globalVar,lines)
	lines= []
...
def main():
	...
	for scalePar in scaleParVec:
		...
		for numleft in range(int(numberOfparallelSims)):
			simInstances.append(jobServer.submit(simulate,(scalePar,meanInterArr,lines),(),("SimPy.Simulation","numpy","numpy.random","scipy.stats","scipy.constants","WrokLoadForAllWithoutMain","csv","string" )))
	...
	file = open('/Volumes/Home/Python_scripts/NetBeansProjects/WrokLoadForContentType/src/workloadInfomationTRTrace.txt','wb')
	w = csv.writer(file)
	...
	file = open('/Volumes/Home/Python_scripts/NetBeansProjects/WrokLoadForContentType/src/queueLengthInfomationTRTrace.txt','wb')
	w = csv.writer(file)
	w.writerow(utilizationVec)
	...
	file = open('/Volumes/Home/Python_scripts/NetBeansProjects/WrokLoadForContentType/src/waitInfomationTRTrace.txt','wb')
	w = csv.writer(file)
	w.writerow(utilizationVec)
	...
	file = open('/Volumes/Home/Python_scripts/NetBeansProjects/WrokLoadForContentType/src/queueSizeInNumberTRTrace.txt','wb')
	w = csv.writer(file)
	w.writerow(utilizationVec)
	...
	file = open('/Volumes/Home/Python_scripts/NetBeansProjects/WrokLoadForContentType/src/service_ArrTimeTRTrace.txt','wb')
	w = csv.writer(file)
	w.writerow(utilizationVec)
...
def getScalePar():
	...
	prevArrTime = 0.0
	InterArrTime = []
	serviceTimeFunc =WrokLoadForAllWithoutMain.ServiceProcess() 
	globalVar = WrokLoadForAllWithoutMain.Global_var()

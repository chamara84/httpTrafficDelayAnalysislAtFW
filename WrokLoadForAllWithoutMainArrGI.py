...
class Arrival(Process):
    ...
    def run(self,globalVar):
    	...
    	self.interArrivalTimeVec = [(float(self.lines[index+1][0])-float(self.lines[index][0]))/float(self.scalepar) for index in range(len(self.lines)-1)]
        ...
        for percentile in  percentileVec:
            self.scores.append(stats.scoreatpercentile(self.interArrivalTimeVec,percentile))
    	
        while 1:
            percentileIndex = int(numpy.floor(globalVar.rand.uniform(low=0.0,high=100.0)/float(0.1)))
            self.interArrivalTime = self.scores[percentileIndex]
            
            yield hold,self,self.interArrivalTime 
            
            if self.ID== len(self.lines)-1:
                print 'Last Arr gen at'+str(globalVar.process.now()) 
...
class Content(Process):
    ...
    def run(self,globalVar):
        ...
        if self.id >= self.numLines-1:
            print 'Stop'
            print 'Inter Arrival Mean Var: '+ str(globalVar.interArrivalTime.mean())+','+str(globalVar.interArrivalTime.var()) 
